...
#elev <- readRDS("I:/projects/BLM/Workspace/kling/shapefiles/elevation_800m_PRISM.rds")
#elev <-resample(elev,dd[[1]], method='bilinear')
...
#type=names(veggies[[1]])

#troubleshooting- stops at 2
#veggies<-veggies[[-c(1,2)]]
names(veggies)
...
r <- foreach(type=names(veggies),
             .packages=c("viridis", "raster", "ecoclim", "dismo", "dplyr", "caret",
                         "randomForest", "ggplot2", "tidyr", "dplyr", "gridExtra", "grid", "here"), .libPaths("C:/Users/patrick_mcintyre/Documents/R/win-library/3.5")) %dopar% {
                               ...
                               dir.create(outdir)
                               #if(length(list.files(outdir)) == 23) return(NULL)
                               ...
                               writeRaster(colstack, paste0(substr(paste0(outdrgb, "/historic_conus_start_change_RGB_", type), 1, 255), ".tif"), overwrite=T) 
                               #plotRGB(colstack)
                               ...
                               png(paste0(substr(paste0(outdir, "/historic_conus_colorwheel_combo_", type), 1, 255), ".png"),
                               ...
                               labs(title=paste0(typename, ":\nbaseline suitability vs suitability change"),
                                    color="current\noccurrence") +
                                     whiteness()
                               ggsave(paste0(substr(paste0(outdir, "/historic_conus_scatter_start_change_", type), 1, 255), ".png"), p, width=6, height=6) 
                               ...
                               ggsave(paste0(substr(paste0(outdir, "/historic_conus_dotplot_delta_", type), 1, 255), ".png"), p, width=8, height=6)  
                               ...
                               for(cull in c(T)){
                                     ...
                                     #ggsave(paste0(substr(paste0(outdir, "/historic_conus_climate_space_scatter_probability_", "cull", cull, "_", type), 1, 255), ".png"), p, width=20, height=12)  
                                     ...
                                     p <- ggplot(ds, aes(x_value, y_value, fill=value)) +
                                           ...
                                           labs(fill="presence   \nprobability   ",
                                                title=paste0(typename, ":\nactual vs. modeled suitability in climate space"))
                                     ggsave(paste0(substr(paste0(outdir, "/historic_conus_climate_space_heatmap_probability_", "cull", cull, "_", type), 1, 255), ".png"), p, width=20, height=12)  
                                     ...
                                     #ggsave(paste0(substr(paste0(outdir, "/historic_conus_climate_space_scatter_delta_", "cull", cull, "_", type), 1, 255), ".png"), p, width=15, height=15)  
                                     ...
                                     ggsave(paste0(substr(paste0(outdir, "/historic_conus_climate_space_heatmap_delta_", "cull", cull, "_", type), 1, 255), ".png"), p, width=15, height=15)  
