...
for(type in names(veggies)) {
      ...
      #type<-names(veggies)[i] # crap from dopar attempt
      print(type)
      
      ### restructure veg data
      veg <- subset(veggies, type) #original
      #veg<-veggies[[1]] #modified for parallel
      ...
      ### set up progress bar
      cullreps <- 19; repreps <- 5; splitreps <- 2
            ...
            for(rep in 1:5){
              ### spatial block cross validation: define folds and then loop through them
                  ...
                  for(split in 1:6){
                        ...
                        set.seed(rep)
                        train_pres <- pixels %>% filter(presence==T & train==T) %>% sample_n(500)#PJM reduced to 500
                        set.seed(rep)
                        train_abs <- pixels %>% filter(presence==F & train==T) %>% sample_n(500)#PJM reduced to 500
                        coordinates(train_pres) <- c("x", "y")
                        coordinates(train_abs) <- c("x", "y")
                        
                        ### select evaluation points
                        evalpx <- min(500, min(table(pixels$presence, pixels$train)[,1]))#PJM reduced to 500
                        set.seed(rep)
...
dataRDS <- readRDS("I:/projects/CEMML_DOD/CEMML_HCCVI/type_specific_modeling/variable_selection/rfe_results/rfe_Central_Tallgrass_Prairie.rds")
