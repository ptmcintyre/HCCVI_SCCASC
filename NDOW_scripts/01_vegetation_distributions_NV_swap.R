...
target_systems<-read.csv(here("S:/Projects/NV_SWAP_HCCVI/selected_LF_systems.csv"), as.is=T)
#following line reads table to select only new adds (for adding in systems)
target_systems<-subset(target_systems, keep==1)
map_values<-target_systems$BPS_CODE
...
CEMML_systems_BPS<-rast("S:/Projects/NV_SWAP_HCCVI/analysis/clip_LF_BPS_wgs84.tif")
...
foreach(i=1:length(map_values), .packages=c("here", "terra")) %dopar% {
  ...
  veg <- classify(focal_system, rclmat, include.lowest=F)
  #veg <- reclassify(focal_system, rcl=c(-1,1,NA,  1,Inf,1), right=F)
  select_system_info<-subset(target_systems, target_systems$BPS_CODE==map_values[i])
  writeRaster(focal_system, filename=here("system_distributions/system_rasters_raw/", paste(select_system_info$BPS_NAME[1],".tif", sep="")), overwrite=T)
}
stopCluster(cl)
