...
required_lib =c('reshape2', 'shapefiles', 'rgdal', 'sp', 'dplyr', 'tidyr');
...
expMetrics<-read.csv(paste0("I:/projects/CEMML_DOD/CEMML_HCCVI/biovar_summaries/base/BASE_allveg_biovar_summaries_bl_",
                     time.period, scenario, ".csv"), as.is=T)
#expMetrics$type<-gsub(" biovar summary bl fut85.csv", "",expMetrics$type )
#expMetrics$type<-gsub(" biovar summary bl near85.csv", "",expMetrics$type )
expMetrics$type<-gsub(" biovar summary bl near45.csv", "",expMetrics$type )
...
# 
# regions<-read.csv("I:/projects/CEMML_DOD/CEMML_HCCVI/system_distributions/d_CEC_Ecoregions.csv", as.is=T)
#regions<-unique(regions[,c("Level3", "Ecoreg_Name")])
#Exposure_regions<-merge(Exposure_systems, regions, by="Ecoreg_Name", all.x=F)
Exposure_regions<-Exposure_systems
...
Exposure_table<-Exposure_regions[,c("Sys ID", "system_name", "NAME", "BioClimVar", "Statistic", "Value")]
Exposure_table$Time_Frame_ID<-Time_Frame

Exposure_table$ScoreDate<-"4/10/2021"
Exposure_table$SourceData<-"MACA_85_CEMML"



Exposure_table<-Exposure_table[,c("Sys ID", "system_name","NAME", "BioClimVar","Time_Frame_ID",  "Statistic", "Value", "SourceData", "ScoreDate")]
...
exposure_50km<-Exposure_table2

percent_areas<-subset(exposure_50km, Statistic=="percent area")
percent_areas<-percent_areas[,c("Sys ID","NAME", "Statistic", "BioClimVar" ,"Value")]
colnames(percent_areas)[colnames(percent_areas)=="Value"]<- "Percent_ecoregion_affected"

exposure50km2<-merge(exposure_50km, percent_areas, by= c("Sys ID", "NAME", "BioClimVar"))
...
write.csv(exposure50km3, paste0("I:/projects/CEMML_DOD/CEMML_HCCVI/summary_table/temp/base_exposure_50km_bl_", time.period,
           scenario, "_all.csv"), row.names=F)


my.data<-read.csv(paste0("I:/projects/CEMML_DOD/CEMML_HCCVI/summary_table/temp/base_exposure_50km_bl_", time.period,
                         scenario, "_all.csv"), as.is=T)
...
colnames(my.data)
#my.cast<-reshape::cast(my.data, system + Bioclim.Variable + Units + NAME+  Sys.ID + ScoreDate + ~ Statistic.x, value="Value")
my.cast<-reshape::cast(my.data, system + Bioclim.Variable + Units + NAME + Sys.ID + ScoreDate  ~ Statistic.x, value="Value")
...
my.cast$Time_Frame_ID<-Time_Frame
colnames(my.cast)  
my.cast2<- my.cast [ , c("Time_Frame_ID", "Scenario", "system", "NAME" , "Bioclim.Variable", "Units",  "mean zscore",
                        "mean(baseline)", "mean delta", "min delta", "max delta", 
                         "Sys.ID", "ScoreDate"  )]


write.csv(my.cast2, paste0("I:/projects/CEMML_DOD/CEMML_HCCVI/summary_table/temp/June24_base_final_table_bl_", time.period, scenario, ".csv"), row.names = F)
