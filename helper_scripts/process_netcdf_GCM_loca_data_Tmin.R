...
clim.files<-list.files("S:/Data/External/Climate/LOCA/CCSM4/rcp85/tasmin/", full.names = T)
clim.files
...
dim(tasmin.array) 
    ...
    r <- raster(t(tasmin.slice), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat), crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0"))
    r<-flip(r)
