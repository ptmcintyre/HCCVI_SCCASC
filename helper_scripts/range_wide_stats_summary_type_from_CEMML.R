...
#sigma_type<-parseMetadata(here("mahalanobis_distance/MD_4vars_2pcs_rp45_bl_kling_method"))
sigma_type<-parseMetadata("I:/projects/CEMML_DOD/CEMML_HCCVI/data_products/intermediate/climate_departure/bl_near_45", pattern="sigma", drop=c("xml", "ovr"))
i=1
...
for (i in 1:length(sigma_type))
  ...
  test.sigma[test.sigma>5]<-5
  test.sigma<-test.sigma/5
  test.sigma<-1-test.sigma
  departure[i]<-cellStats(test.sigma, stat='mean', na.rm=T)
  stat.names[i]<-names(test.sigma)
  
}

suitability_df<-cbind(stat.names, departure)

write.csv(suitability_df, "I:/projects/CEMML_DOD/CEMML_HCCVI/GMU_project/departure_CEMML.csv")

sigma_type<-parseMetadata("I:/projects/CEMML_DOD/CEMML_HCCVI/data_products/intermediate/change_suitability/near_45", pattern="tif", drop=c("xml", "ovr"))
i=1
i=1
#my.sigma.hist<-png("I:/projects/CEMML_DOD/CEMML_HCCVI/mahalanobis_distance/Type_spec_6vars_2pcs_type_BL_fut85.png",  width=16, height=20, units="in", res=1000)
#par(mfrow = c(8, 3))
...
i=1
for (i in 1:length(sigma_type$path)){
  test.sigma<-raster(sigma_type$path[i])
  test.sigma[test.sigma>0]<-0
  test.sigma<-test.sigma +1
  extent(test.sigma)
  #test.sigma<-crop(veggies[[i]],test.sigma)
  #test.sigma<-mask(test.sigma, veggies[[i]])
  veg.crop<-crop(veggies[[i]],test.sigma)
  test.sigma<-mask(test.sigma, veg.crop)
  suitability[i]<-cellStats(test.sigma, stat='mean', na.rm=T)
  stat.names[i]<-names(test.sigma)
  
}

suitability_df<-cbind(stat.names, suitability)
