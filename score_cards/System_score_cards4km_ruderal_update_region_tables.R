...
veg_rasters
veg_rasters<- veg_rasters[c(3,23)]
...
inputs_nr45<-inputs_nr45[c(2,18),]
inputs_nr85<-inputs_nr85[c(2, 18),]
inputs_fut45<-inputs_fut45[c(2,18),]
inputs_fut85<-inputs_fut85[c(2, 18),]
...
dep_nr45<-dep_nr45[c(18)]
dep_nr85<-dep_nr85[c(18 )]
dep_fut45<-dep_fut45[c(18)]
dep_fut85<-dep_fut85[c(18)]
...
system_input_table<-system_input_table[c(23),]
system_input_table
...
selected_system<-gsub('.tif','', selected_system)
...
#maca_poly<-st_read(paste0("I:/projects/CEMML_DOD/CEMML_HCCVI/system_distributions/system_rasters_raw_evt/poly/", type, ".shp"))

focal_tri<-exact_extract(tri, maca_poly, 'mean')
maca_poly$focal_tri<-focal_tri
focal_tri<-fasterize(maca_poly, veggies[[i]], field='focal_tri', fun='last') 
focal_tri<-exact_extract(focal_tri, maca_focal_aea, 'mean')
focal_tri<-round((focal_tri/100),3)
maca_focal_aea$TRI<-focal_tri

focal_land<-exact_extract(land_cond, maca_poly, 'mean')
maca_poly$focal_land<-focal_land
focal_land<-fasterize(maca_poly, veggies[[i]], field='focal_land', fun='last') 
...
focal_Fire_VDep<-exact_extract(Fire_VDep, maca_poly, 'mean')
maca_poly$focal_Fire_VDep<-focal_Fire_VDep
focal_Fire_VDep<-fasterize(maca_poly, veggies[[i]], field='focal_Fire_VDep', fun='last') 
...
#check scoring
  ...
  if(system_input$Forest_Insect_disease=="yes"){
    focal_fld<-exact_extract(ldloss, maca_poly, 'mean')
    maca_poly$focal_fld<-focal_fld
    focal_fld<-fasterize(maca_poly, veggies[[i]], field='focal_fld', fun='last') 
...
write.csv(trans_data, paste0("I:/projects/CEMML_DOD/CEMML_HCCVI/data_products/", type, "_Region_Score_Card_7_23_EVT.csv"))
